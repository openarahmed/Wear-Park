...
export default function DashboardPage() {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await fetch("https://server-wear-park.vercel.app/products");
    ...
    const handleUpdateProduct = async (data: Product) => {
        ...
        try {
            ...
            // create Product
            const response = await fetch(`https://server-wear-park.vercel.app/products/${editProduct?._id}`, {
                method: "PUT",
                headers: {
            ...
            console.error("Error in create/update product:", error); // Debugging line
        }
    };
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    const response = await fetch(`https://server-wear-park.vercel.app/products/${id}`, {
                        method: "DELETE",
                        headers: {
                            "Content-Type": "application/json",
                            "Authorization": token,
    ...
    return (
        <div className="p-6">
            ...
            {/* Edit Product */}
            <Dialog open={isUpdateModalOpen} onOpenChange={(open) => setIsUpdateModalOpen(open)}>
                <DialogContent>
                    ...
                    <form onSubmit={handleSubmit(handleUpdateProduct)} className="space-y-4">
                        ...
                        <Button type="submit" className={editProduct ? "" : "hidden"}>Update</Button>
                    </form>
                </DialogContent>
            </Dialog>
